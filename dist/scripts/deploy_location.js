...
exports.deployLocationRegistry = deployLocationRegistry;
    ...
    defaultGridZones: [
        {
            ...
            restrictions: {
                ...
                allowedLocationTypes: [LocationStructure_1.LocationType.PRODUCER, LocationStructure_1.LocationType.CONSUMER, LocationStructure_1.LocationType.PROSUMER],
 ...
 */
async function deployLocationRegistry() {
    ...
    try {
        ...
        const locationRegistry = new LocationRegistry_1.LocationRegistry();
        ...
        const pricingZoneIds = [];
        for (const pricingConfig of DEPLOY_CONFIG.defaultPricingZones) {
            const pricingZoneId = await locationRegistry.createPricingZone(pricingConfig.name, pricingConfig.basePrice, pricingConfig.peakMultiplier, pricingConfig.offPeakMultiplier, pricingConfig.regionalAdjustment, DEPLOY_CONFIG.adminAddress);
        ...
        for (let i = 0; i < DEPLOY_CONFIG.defaultGridZones.length; i++) {
            ...
            const gridZoneId = await locationRegistry.createGridZone(gridConfig.name, gridConfig.boundaries, pricingZoneId, gridConfig.restrictions, DEPLOY_CONFIG.adminAddress);
        ...
        const sampleLocations = [
            {
                ...
                type: LocationStructure_1.LocationType.PRODUCER,
                privacy: LocationStructure_1.PrivacyLevel.PUBLIC,
            ...
            },
            {
                ...
                type: LocationStructure_1.LocationType.CONSUMER,
                privacy: LocationStructure_1.PrivacyLevel.PRIVACY_ZONE,
            ...
            },
            {
                ...
                type: LocationStructure_1.LocationType.CONSUMER,
                privacy: LocationStructure_1.PrivacyLevel.PRIVATE,
                metadata: 'High-density data center in Tokyo'
            },
            {
                name: 'Texas Solar Prosumer',
                coordinates: { latitude: 32.7767, longitude: -96.7970 },
                type: LocationStructure_1.LocationType.PROSUMER,
                privacy: LocationStructure_1.PrivacyLevel.PUBLIC,
                metadata: 'Residential solar installation with battery storage'
            }
        ];
        const sampleLocationIds = [];
        for (const sample of sampleLocations) {
            const locationId = await locationRegistry.registerLocation(sample.coordinates, sample.type, sample.privacy, sample.metadata, 'sample_user');
        ...
        for (const locationId of sampleLocationIds) {
            await locationRegistry.verifyLocation(locationId, LocationStructure_1.VerificationMethod.GPS_COORDINATES, 'Automated verification during deployment', DEPLOY_CONFIG.adminAddress);
...
async function verifyDeployment() {
    ...
    try {
        ...
        const locationId = await locationRegistry.registerLocation(testCoordinates, LocationStructure_1.LocationType.PRODUCER, LocationStructure_1.PrivacyLevel.PUBLIC, 'Verification test location', 'verifier');
...
//# sourceMappingURL=deploy_location.js.map