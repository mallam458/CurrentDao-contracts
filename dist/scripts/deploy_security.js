...
exports.SecurityDeployer = void 0;
...
class SecurityDeployer {
    ...
    async deploy() {
        ...
        try {
            ...
            const accessControl = new AccessControl_1.AccessControl();
            // Setup initial roles
     ...
     */
    async setupInitialRoles(accessControl) {
     ...
     */
    async setupPermissions(accessControl) {
        console.log("🔐 Setting up permissions...");
        // Admin permissions (already set in constructor, but ensuring all are set)
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_ADMIN, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_EMERGENCY, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_GRANT_ROLE, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_REVOKE_ROLE, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_SET_PERMISSION, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_MULTISIG_ADMIN, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_MINT, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_BURN, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_TRANSFER, true);
        await accessControl.setPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_PAUSE, true);
        // Operator permissions
        await accessControl.setPermission(IAccessControl_1.OPERATOR_ROLE, IAccessControl_1.PERMISSION_MINT, true);
        await accessControl.setPermission(IAccessControl_1.OPERATOR_ROLE, IAccessControl_1.PERMISSION_BURN, true);
        await accessControl.setPermission(IAccessControl_1.OPERATOR_ROLE, IAccessControl_1.PERMISSION_TRANSFER, true);
        await accessControl.setPermission(IAccessControl_1.OPERATOR_ROLE, IAccessControl_1.PERMISSION_PAUSE, true);
        // User permissions
        await accessControl.setPermission(IAccessControl_1.USER_ROLE, IAccessControl_1.PERMISSION_TRANSFER, true);
     ...
     */
    async setupMultiSigRequirements(accessControl) {
     ...
     */
    async grantInitialRoles(accessControl) {
        console.log("👥 Granting initial roles...");
        // Grant admin role to admin address
        await accessControl.grantRole(IAccessControl_1.DEFAULT_ADMIN_ROLE, this.config.adminAddress);
        ...
        for (const operator of this.config.initialOperators) {
            await accessControl.grantRole(IAccessControl_1.OPERATOR_ROLE, operator);
            console.log(`  - Operator role granted to ${operator}`);
        }
        // Grant user roles
        for (const user of this.config.initialUsers) {
            await accessControl.grantRole(IAccessControl_1.USER_ROLE, user);
     ...
     */
    async verifyDeployment(accessControl) {
        console.log("🔍 Verifying deployment...");
        try {
            // Check if all default roles exist
            const allRoles = accessControl.getAllRoles();
            const requiredRoles = [IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.OPERATOR_ROLE, IAccessControl_1.USER_ROLE, IAccessControl_1.VIEWER_ROLE];
            ...
            const hasAdminRole = await accessControl.hasRole(IAccessControl_1.DEFAULT_ADMIN_ROLE, this.config.adminAddress);
            if (!hasAdminRole) {
                console.error("❌ Admin does not have admin role");
                return false;
            }
            // Check permissions are set correctly
            const hasAdminPermission = await accessControl.hasPermission(IAccessControl_1.DEFAULT_ADMIN_ROLE, IAccessControl_1.PERMISSION_ADMIN, this.config.adminAddress);
...
exports.SecurityDeployer = SecurityDeployer;
/**
    ...
    const configs = {
        development: {
            ...
            multiSigRequirements: new Map([
                [IAccessControl_1.PERMISSION_MINT, 2],
                [IAccessControl_1.PERMISSION_BURN, 2],
                [IAccessControl_1.PERMISSION_EMERGENCY, 3],
                [IAccessControl_1.PERMISSION_ADMIN, 3]
            ])
        },
        testnet: {
            network: "testnet",
            ...
            multiSigRequirements: new Map([
                [IAccessControl_1.PERMISSION_MINT, 2],
                [IAccessControl_1.PERMISSION_BURN, 2],
                [IAccessControl_1.PERMISSION_EMERGENCY, 2],
                [IAccessControl_1.PERMISSION_ADMIN, 2]
            ])
        },
        mainnet: {
            network: "mainnet",
            ...
            multiSigRequirements: new Map([
                [IAccessControl_1.PERMISSION_MINT, 3],
                [IAccessControl_1.PERMISSION_BURN, 3],
                [IAccessControl_1.PERMISSION_EMERGENCY, 5],
                [IAccessControl_1.PERMISSION_ADMIN, 5],
                [IAccessControl_1.PERMISSION_PAUSE, 2]
            ])
        }
    };
 ...
 */
function createConfigFromEnv(network) {
    return {
        network,
...
//# sourceMappingURL=deploy_security.js.map