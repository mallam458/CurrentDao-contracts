...
const IAccessControlList_1 = require("../../contracts/acl/interfaces/IAccessControlList");
describe('AccessControlList', () => {
    const governanceMembers = ['0xgov1', '0xgov2', '0xgov3'];
    const emergencyCouncil = ['0xemergency'];
    let acl;
    beforeEach(() => {
        acl = new AccessControlList_1.AccessControlList({
            config: {
                governanceMembers,
                emergencyCouncil,
    ...
    it('allows direct permissions to override role access', async () => {
        ...
        await acl.grantDirectPermission('0xuser', 'EXECUTE_TREASURY_WITHDRAW', '0xbootstrap', IAccessControlList_1.AccessEffect.DENY, 'Freeze withdrawals');
    ...
    it('creates and executes governance-managed dynamic roles with community input', async () => {
        const proposal = await acl.createGovernanceProposal(IAccessControlList_1.GovernanceProposalType.CREATE_ROLE, 'Create Integrations Role', 'Allows controlled registry updates', governanceMembers[0], {
        ...
        await acl.castGovernanceVote(proposal.id, governanceMembers[0], IAccessControlList_1.GovernanceVoteChoice.FOR, 'Needed');
        await acl.castGovernanceVote(proposal.id, governanceMembers[1], IAccessControlList_1.GovernanceVoteChoice.FOR, 'Agreed');
    ...
    it('supports emergency function locks and global lockdown', async () => {
        ...
        await acl.executeEmergencyAction({
            action: IAccessControlList_1.EmergencyActionType.LOCK_FUNCTION,
        ...
        await acl.executeEmergencyAction({
            action: IAccessControlList_1.EmergencyActionType.ENABLE_GLOBAL_LOCKDOWN,
...
//# sourceMappingURL=AccessControlList.test.js.map