...
const BatchProcessor_1 = require("../../contracts/batching/BatchProcessor");
const BatchStructure_1 = require("../../contracts/batching/structures/BatchStructure");
const BatchingLib_1 = require("../../contracts/batching/libraries/BatchingLib");
describe('BatchProcessor', () => {
    ...
    beforeEach(() => {
        ...
        batchProcessor = new BatchProcessor_1.BatchProcessor(mockConfig);
    });
    describe('Batch Creation', () => {
        test('should create a batch with valid transactions', () => {
            ...
            expect(batch.status).toBe(BatchStructure_1.BatchStatus.PENDING);
        });
        test('should throw error when creating batch with no valid transactions', () => {
            const invalidTransactions = [
                createMockTransaction('', BatchStructure_1.TransactionType.TRANSFER) // Empty from address
            ];
            expect(() => {
        ...
        test('should set correct batch priority based on highest priority transaction', () => {
            const transactions = [
                createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER, BatchStructure_1.TransactionPriority.LOW),
                createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER, BatchStructure_1.TransactionPriority.HIGH),
                createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER, BatchStructure_1.TransactionPriority.MEDIUM)
            ];
            const batchId = batchProcessor.createBatch(transactions, '0x123');
            const batch = batchProcessor.getBatch(batchId);
            expect(batch.priority).toBe(BatchStructure_1.TransactionPriority.HIGH);
        });
    });
    describe('Transaction Management', () => {
        ...
        test('should add transaction to pending batch', () => {
            const newTransaction = createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER);
    ...
    describe('Batch Validation', () => {
        ...
        test('should fail validation for batch with invalid transactions', () => {
            ...
            const mixedTransactions = [
                createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER), // Valid
                createMockTransaction('', BatchStructure_1.TransactionType.TRANSFER) // Invalid - empty from address
            ];
            ...
            batch.transactions.push(createMockTransaction('', BatchStructure_1.TransactionType.TRANSFER));
    ...
    describe('Batch Execution', () => {
        ...
        test('should execute batch successfully', async () => {
            ...
            const batch = batchProcessor.getBatch(batchId);
            expect(batch.status).toBe(BatchStructure_1.BatchStatus.COMPLETED);
        });
        test('should execute batch with priority', async () => {
            const result = await batchProcessor.executeBatchWithPriority(batchId, BatchStructure_1.TransactionPriority.HIGH);
            expect(result.success).toBe(true);
            expect(result.executedTransactions).toHaveLength(5);
        });
        test('should handle execution failure and rollback', async () => {
            ...
            const failingTransactions = [
                createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER),
                createMockTransaction('0x123', BatchStructure_1.TransactionType.TRANSFER)
            ];
            // Override the executeTransaction method to simulate failure
            const originalExecute = batchProcessor.executeTransaction;
            batchProcessor.executeTransaction = jest.fn().mockImplementation(() => {
            ...
            batchProcessor.executeTransaction = originalExecute;
        });
    ...
    describe('Emergency Functions', () => {
        ...
        test('should emergency cancel batch', () => {
            ...
            const batch = batchProcessor.getBatch(batchId);
            expect(batch.status).toBe(BatchStructure_1.BatchStatus.CANCELLED);
        ...
        test('should rollback failed batch', async () => {
            ...
            const updatedBatch = batchProcessor.getBatch(batchId);
            expect(updatedBatch.status).toBe(BatchStructure_1.BatchStatus.ROLLED_BACK);
        });
    });
    ...
    describe('Status and Monitoring', () => {
        ...
        test('should get batch status', () => {
            ...
            expect(status).toBe(BatchStructure_1.BatchStatus.PENDING);
        });
        ...
        test('should get transactions by priority', () => {
            const highPriorityTxs = batchProcessor.getTransactionsByPriority(batchId, BatchStructure_1.TransactionPriority.MEDIUM);
            expect(highPriorityTxs).toBeDefined();
            expect(Array.isArray(highPriorityTxs)).toBe(true);
        });
        test('should get batches by status', () => {
            const pendingBatches = batchProcessor.getBatches(BatchStructure_1.BatchStatus.PENDING);
            expect(pendingBatches).toHaveLength(1);
            expect(pendingBatches[0].status).toBe(BatchStructure_1.BatchStatus.PENDING);
        });
...
function createMockTransactions(count) {
    ...
    for (let i = 0; i < count; i++) {
        transactions.push(createMockTransaction(`0x${i.toString().padStart(40, '0')}`, BatchStructure_1.TransactionType.TRANSFER, BatchStructure_1.TransactionPriority.MEDIUM));
    }
    return transactions;
}
function createMockTransaction(from, type = BatchStructure_1.TransactionType.TRANSFER, priority = BatchStructure_1.TransactionPriority.MEDIUM) {
    const timestamp = Date.now();
    const nonce = Math.floor(Math.random() * 1000000);
    return {
        id: BatchingLib_1.BatchingLib.generateTransactionId(from, timestamp, nonce),
        type,
        from,
...
//# sourceMappingURL=BatchProcessor.test.js.map