...
const YieldFarming_1 = require("../../contracts/staking/YieldFarming");
const StakingLib_1 = require("../../contracts/staking/libraries/StakingLib");
const RewardCalculator_1 = require("../../contracts/staking/rewards/RewardCalculator");
...
function makeYF(startTime = 1_000_000) {
    let t = startTime;
    const clock = () => t;
    const yf = new YieldFarming_1.YieldFarming(1_000_000, clock);
    return { yf, clock, advance: (secs) => { t += secs; } };
}
// ─── Pool Management ────────────────────────────────────────────────────────
...
describe('Compounding', () => {
    ...
    test('compounding boost is ~15% annually over simple interest', () => {
        ...
        const boosted = StakingLib_1.StakingLib.applyCompoundBoost(base, periods);
...
describe('RewardCalculator', () => {
    const calc = new RewardCalculator_1.RewardCalculator();
...
describe('StakingLib', () => {
    test('validateLockup accepts valid range', () => {
        expect(StakingLib_1.StakingLib.validateLockup(ONE_DAY)).toBe(true);
        expect(StakingLib_1.StakingLib.validateLockup(ONE_YEAR)).toBe(true);
        expect(StakingLib_1.StakingLib.validateLockup(ONE_DAY - 1)).toBe(false);
        expect(StakingLib_1.StakingLib.validateLockup(ONE_YEAR + 1)).toBe(false);
    });
    test('calcUtilization clamps at 100%', () => {
        expect(StakingLib_1.StakingLib.calcUtilization(2000, 1000)).toBe(100);
        expect(StakingLib_1.StakingLib.calcUtilization(500, 1000)).toBe(50);
    });
    test('estimateGasCategory returns low for claim', () => {
        expect(StakingLib_1.StakingLib.estimateGasCategory('claim')).toBe('low');
    });
    test('applyCompoundBoost increases rewards', () => {
        const boosted = StakingLib_1.StakingLib.applyCompoundBoost(1000, 12);
        expect(boosted).toBeGreaterThan(1000);
    });
});
//# sourceMappingURL=YieldFarming.test.js.map