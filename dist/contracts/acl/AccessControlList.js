"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AccessControlList = void 0;
const IAccessControlList_1 = require("./interfaces/IAccessControlList");
const ACLGovernance_1 = require("./governance/ACLGovernance");
const ACLLib_1 = require("./libraries/ACLLib");
const PermissionManager_1 = require("./permissions/PermissionManager");
const RoleManager_1 = require("./roles/RoleManager");
const PLATFORM_FUNCTIONS = {
    DAO: ['propose', 'vote', 'execute', 'configure'],
    TREASURY: ['withdraw', 'deposit', 'rebalance', 'settle'],
    STAKING: ['stake', 'unstake', 'claimRewards', 'configurePool'],
    LIQUIDITY: ['addLiquidity', 'removeLiquidity', 'rebalancePool', 'bridge'],
    REGISTRY: ['register', 'update', 'remove', 'resolve'],
    REGULATORY: ['submitReport', 'approveReport', 'archiveReport'],
    SECURITY: ['pause', 'unpause', 'rotateKey', 'reviewIncident'],
    FEES: ['setFee', 'switchModel', 'sweepFees'],
    ORACLE: ['pushPrice', 'freezeFeed', 'resumeFeed'],
    ZKP: ['verifyProof', 'revealTradeDetails']
};
const PREDEFINED_ROLE_BLUEPRINTS = [
    { id: 'SUPER_ADMIN', label: 'Super Admin', description: 'Platform-wide root administrator.' },
    { id: 'DAO_EXECUTIVE', label: 'DAO Executive', description: 'Executes approved DAO actions.', parents: ['SUPER_ADMIN'] },
    { id: 'DAO_GOVERNOR', label: 'DAO Governor', description: 'Creates and manages DAO governance flows.', parents: ['DAO_EXECUTIVE'] },
    { id: 'TREASURY_MANAGER', label: 'Treasury Manager', description: 'Manages treasury strategy.', parents: ['DAO_EXECUTIVE'] },
    { id: 'TREASURY_OPERATOR', label: 'Treasury Operator', description: 'Performs treasury operations.', parents: ['TREASURY_MANAGER'] },
    { id: 'COMPLIANCE_ADMIN', label: 'Compliance Admin', description: 'Owns compliance controls.', parents: ['DAO_EXECUTIVE'] },
    { id: 'COMPLIANCE_OFFICER', label: 'Compliance Officer', description: 'Runs day-to-day compliance operations.', parents: ['COMPLIANCE_ADMIN'] },
    { id: 'RISK_MANAGER', label: 'Risk Manager', description: 'Oversees protocol risk.', parents: ['DAO_EXECUTIVE'] },
    { id: 'SECURITY_ADMIN', label: 'Security Admin', description: 'Owns security posture and access hardening.', parents: ['DAO_EXECUTIVE'] },
    { id: 'SECURITY_ANALYST', label: 'Security Analyst', description: 'Monitors and reviews security operations.', parents: ['SECURITY_ADMIN'] },
    { id: 'EMERGENCY_RESPONDER', label: 'Emergency Responder', description: 'Can react to security incidents immediately.', parents: ['SECURITY_ADMIN'] },
    { id: 'ORACLE_MANAGER', label: 'Oracle Manager', description: 'Manages oracle feeds.', parents: ['DAO_EXECUTIVE'] },
    { id: 'ORACLE_OPERATOR', label: 'Oracle Operator', description: 'Operates oracle feeds.', parents: ['ORACLE_MANAGER'] },
    { id: 'REGISTRY_ADMIN', label: 'Registry Admin', description: 'Maintains contract registry.', parents: ['DAO_EXECUTIVE'] },
    { id: 'LIQUIDITY_MANAGER', label: 'Liquidity Manager', description: 'Owns liquidity strategy.', parents: ['DAO_EXECUTIVE'] },
    { id: 'LIQUIDITY_OPERATOR', label: 'Liquidity Operator', description: 'Executes liquidity operations.', parents: ['LIQUIDITY_MANAGER'] },
    { id: 'STAKING_MANAGER', label: 'Staking Manager', description: 'Owns staking policy.', parents: ['DAO_EXECUTIVE'] },
    { id: 'STAKING_OPERATOR', label: 'Staking Operator', description: 'Executes staking actions.', parents: ['STAKING_MANAGER'] },
    { id: 'FEE_MANAGER', label: 'Fee Manager', description: 'Owns fee policy.', parents: ['DAO_EXECUTIVE'] },
    { id: 'FEE_OPERATOR', label: 'Fee Operator', description: 'Operates fee updates.', parents: ['FEE_MANAGER'] },
    { id: 'REPORTING_MANAGER', label: 'Reporting Manager', description: 'Oversees reporting and disclosures.', parents: ['COMPLIANCE_ADMIN'] },
    { id: 'READ_ONLY_OBSERVER', label: 'Read Only Observer', description: 'Non-destructive read access role.' },
    { id: 'AUDITOR', label: 'Auditor', description: 'Read-only audit access across modules.', parents: ['READ_ONLY_OBSERVER'] },
    { id: 'SUPPORT_AGENT', label: 'Support Agent', description: 'Limited customer support operations.', parents: ['READ_ONLY_OBSERVER'] }
];
class AccessControlList {
    roleManager = new RoleManager_1.RoleManager();
    permissionManager = new PermissionManager_1.PermissionManager();
    governance;
    eventHandlers;
    auditTrail = [];
    accessMeasurements = [];
    config;
    emergencyState = {
        active: false,
        lockedFunctions: []
    };
    constructor(options = {}) {
        this.config = {
            ...IAccessControlList_1.DEFAULT_ACL_CONFIG,
            ...options.config,
            emergencyCouncil: [...(options.config?.emergencyCouncil || IAccessControlList_1.DEFAULT_ACL_CONFIG.emergencyCouncil)],
            governanceMembers: [...(options.config?.governanceMembers || IAccessControlList_1.DEFAULT_ACL_CONFIG.governanceMembers)]
        };
        this.eventHandlers = options.eventHandlers || {};
        this.governance = new ACLGovernance_1.ACLGovernance(this.config.governanceMembers);
        this.bootstrapPermissions(options.bootstrapActor || 'SYSTEM');
        this.bootstrapRoles(options.bootstrapActor || 'SYSTEM');
    }
    async hasAccess(request) {
        const functionKey = ACLLib_1.ACLLib.buildPermissionKey(request.contractId, request.functionName);
        const effectiveRoleIds = this.roleManager.getEffectiveRoleIds(request.subject);
        const mergedPermissionIds = ACLLib_1.ACLLib.mergePermissions(effectiveRoleIds, this.roleManager.getRoleStore());
        const inheritedRoleIds = effectiveRoleIds.filter((roleId) => !this.roleManager.getAssignedRoles(request.subject).some((assignment) => assignment.roleId === roleId));
        const directGrants = this.permissionManager.getDirectGrants(request.subject);
        const directMatches = directGrants
            .map((grant) => ({
            grant,
            permission: this.permissionManager.getPermission(grant.permissionId)
        }))
            .filter((item) => Boolean(item.permission))
            .filter((item) => ACLLib_1.ACLLib.permissionMatches(item.permission, request));
        const matchedRolePermissions = this.permissionManager.matchPermissions(mergedPermissionIds, request.contractId, request.functionName);
        const lockedFunction = this.emergencyState.lockedFunctions.includes(functionKey) || this.emergencyState.lockedFunctions.includes(`${request.contractId}::*`);
        const matchedPermissionCount = matchedRolePermissions.length + directMatches.length;
        const gasEstimate = ACLLib_1.ACLLib.estimateAccessCheckGas({
            roleCount: this.roleManager.getAssignedRoles(request.subject).length,
            inheritedRoleCount: inheritedRoleIds.length,
            directGrantCount: directGrants.length,
            locked: this.emergencyState.active || lockedFunction,
            matchedPermissionCount
        });
        let decision;
        if (this.emergencyState.active) {
            decision = ACLLib_1.ACLLib.buildDeniedDecision(IAccessControlList_1.ACL_ERROR.EMERGENCY_LOCKDOWN, gasEstimate);
        }
        else if (lockedFunction) {
            decision = ACLLib_1.ACLLib.buildDeniedDecision(IAccessControlList_1.ACL_ERROR.FUNCTION_LOCKED, gasEstimate);
        }
        else if (directMatches.some(({ grant }) => grant.effect === IAccessControlList_1.AccessEffect.DENY)) {
            decision = ACLLib_1.ACLLib.buildDeniedDecision(IAccessControlList_1.ACL_ERROR.ACCESS_DENIED, gasEstimate);
        }
        else if (matchedRolePermissions.some((permission) => permission.effect === IAccessControlList_1.AccessEffect.DENY)) {
            decision = ACLLib_1.ACLLib.buildDeniedDecision(IAccessControlList_1.ACL_ERROR.ACCESS_DENIED, gasEstimate);
        }
        else if (directMatches.some(({ grant }) => grant.effect === IAccessControlList_1.AccessEffect.ALLOW) || matchedRolePermissions.some((permission) => permission.effect === IAccessControlList_1.AccessEffect.ALLOW)) {
            decision = ACLLib_1.ACLLib.buildDecision(true, 'ACCESS_GRANTED', [...matchedRolePermissions.map((permission) => permission.id), ...directMatches.map(({ grant }) => grant.permissionId)], inheritedRoleIds, gasEstimate);
        }
        else {
            decision = ACLLib_1.ACLLib.buildDeniedDecision(IAccessControlList_1.ACL_ERROR.ACCESS_DENIED, gasEstimate);
        }
        ACLLib_1.ACLLib.assertGasTarget(gasEstimate, this.config.maxAccessCheckGas);
        this.recordAccessMeasurement(gasEstimate);
        this.logAudit(IAccessControlList_1.AuditAction.ACCESS_CHECK, request.actor || request.subject, decision.allowed, decision.reason, {
            contractId: request.contractId,
            functionName: request.functionName,
            matchedPermissionCount,
            inheritedRoleCount: inheritedRoleIds.length,
            subject: request.subject
        }, {
            subject: request.subject,
            contractId: request.contractId,
            functionName: request.functionName,
            targetId: functionKey
        });
        return decision;
    }
    async assignRole(subject, roleId, actor, reason, expiresAt) {
        const assignment = this.roleManager.assignRole(subject, roleId, actor, reason, expiresAt);
        this.logAudit(IAccessControlList_1.AuditAction.ROLE_ASSIGNED, actor, true, reason || 'Role assigned', { roleId: assignment.roleId, expiresAt: expiresAt || 0 }, { subject, targetId: assignment.roleId });
        this.eventHandlers.onRoleAssigned?.(assignment);
    }
    async revokeRole(subject, roleId, actor, reason) {
        const revoked = this.roleManager.revokeRole(subject, roleId);
        this.logAudit(IAccessControlList_1.AuditAction.ROLE_REVOKED, actor, Boolean(revoked), reason || (revoked ? 'Role revoked' : 'Role assignment not found'), { roleId: ACLLib_1.ACLLib.normalizeId(roleId) }, { subject, targetId: ACLLib_1.ACLLib.normalizeId(roleId) });
        if (revoked) {
            this.eventHandlers.onRoleRevoked?.(revoked);
        }
    }
    async createPermission(payload, actor) {
        const permission = this.permissionManager.createPermission(payload, actor);
        this.logAudit(IAccessControlList_1.AuditAction.PERMISSION_CREATED, actor, true, 'Permission created', { contractId: permission.contractId, functionName: permission.functionName }, { targetId: permission.id, contractId: permission.contractId, functionName: permission.functionName });
        return permission;
    }
    async createGovernanceProposal(type, title, description, proposer, payload, executionDelaySeconds = 0) {
        const proposal = this.governance.createProposal(type, title, description, proposer, payload, executionDelaySeconds);
        this.logAudit(IAccessControlList_1.AuditAction.GOVERNANCE_PROPOSAL_CREATED, proposer, true, 'Governance proposal created', { type }, { targetId: proposal.id });
        return proposal;
    }
    async executeGovernanceProposal(proposalId, executor) {
        const proposal = this.governance.getProposal(proposalId);
        const evaluation = this.governance.evaluateProposal(proposalId, this.config.governanceThreshold, this.config.communityQuorum);
        if (!evaluation.approved) {
            throw new Error(IAccessControlList_1.ACL_ERROR.PROPOSAL_NOT_APPROVED);
        }
        await this.applyGovernancePayload(proposal);
        const executed = this.governance.markExecuted(proposalId);
        this.logAudit(IAccessControlList_1.AuditAction.GOVERNANCE_PROPOSAL_EXECUTED, executor, true, 'Governance proposal executed', { type: executed.type }, { targetId: proposalId });
        this.eventHandlers.onGovernanceExecuted?.(executed);
    }
    async getAuditTrail(subject) {
        return this.auditTrail
            .filter((entry) => !subject || entry.subject === subject || entry.actor === subject)
            .map((entry) => ({ ...entry, metadata: { ...entry.metadata } }));
    }
    async getPerformanceSnapshot() {
        if (this.accessMeasurements.length === 0) {
            return {
                averageAccessCheckGas: 0,
                peakAccessCheckGas: 0,
                totalChecks: 0,
                withinTarget: true
            };
        }
        const total = this.accessMeasurements.reduce((sum, entry) => sum + entry.gasEstimate, 0);
        const peak = this.accessMeasurements.reduce((max, entry) => Math.max(max, entry.gasEstimate), 0);
        return {
            averageAccessCheckGas: Math.round(total / this.accessMeasurements.length),
            peakAccessCheckGas: peak,
            totalChecks: this.accessMeasurements.length,
            withinTarget: peak <= this.config.maxAccessCheckGas
        };
    }
    async createRole(payload, actor, governanceManaged = false) {
        const role = this.roleManager.createRole(payload, actor, false, governanceManaged);
        this.logAudit(IAccessControlList_1.AuditAction.ROLE_CREATED, actor, true, 'Role created', { governanceManaged }, { targetId: role.id });
        return role;
    }
    async updateRole(payload, actor) {
        const role = this.roleManager.updateRole(payload);
        this.logAudit(IAccessControlList_1.AuditAction.ROLE_CREATED, actor, true, 'Role updated', {}, { targetId: role.id });
        return role;
    }
    async assignPermissionToRole(payload, actor) {
        const role = this.roleManager.addPermissionToRole(payload.roleId, payload.permissionId);
        this.logAudit(IAccessControlList_1.AuditAction.PERMISSION_ASSIGNED, actor, true, 'Permission assigned to role', { permissionId: ACLLib_1.ACLLib.normalizeId(payload.permissionId) }, { targetId: role.id });
        return role;
    }
    async grantDirectPermission(subject, permissionId, actor, effect, reason, expiresAt) {
        const grant = this.permissionManager.grantDirectPermission(subject, permissionId, actor, effect, reason, expiresAt);
        this.logAudit(effect === IAccessControlList_1.AccessEffect.ALLOW ? IAccessControlList_1.AuditAction.DIRECT_PERMISSION_GRANTED : IAccessControlList_1.AuditAction.DIRECT_PERMISSION_DENIED, actor, true, reason || 'Direct permission grant updated', { effect, expiresAt: expiresAt || 0 }, { subject, targetId: grant.permissionId });
    }
    async revokeDirectPermission(subject, permissionId, actor) {
        const revoked = this.permissionManager.revokeDirectPermission(subject, permissionId);
        this.logAudit(IAccessControlList_1.AuditAction.DIRECT_PERMISSION_DENIED, actor, Boolean(revoked), revoked ? 'Direct permission revoked' : 'Direct permission not found', {}, { subject, targetId: ACLLib_1.ACLLib.normalizeId(permissionId) });
    }
    async registerContractFunctions(contractId, functions, actor) {
        const permissions = this.permissionManager.registerFunctionPermissions(contractId, functions, actor);
        permissions.forEach((permission) => this.logAudit(IAccessControlList_1.AuditAction.PERMISSION_CREATED, actor, true, 'Function permission registered', {}, { targetId: permission.id, contractId: permission.contractId, functionName: permission.functionName }));
        return permissions;
    }
    async castGovernanceVote(proposalId, voter, choice, reason) {
        const proposal = this.governance.castVote(proposalId, voter, choice, reason);
        this.logAudit(IAccessControlList_1.AuditAction.GOVERNANCE_VOTE_CAST, voter, true, reason || 'Governance vote cast', { choice }, { targetId: proposalId });
        return proposal;
    }
    async submitCommunityFeedback(proposalId, author, support, comment) {
        return this.governance.submitCommunityFeedback(proposalId, author, support, comment);
    }
    async evaluateGovernanceProposal(proposalId) {
        return this.governance.evaluateProposal(proposalId, this.config.governanceThreshold, this.config.communityQuorum);
    }
    async executeEmergencyAction(payload, actor) {
        if (!this.isEmergencyAuthorized(actor)) {
            throw new Error(IAccessControlList_1.ACL_ERROR.ACCESS_DENIED);
        }
        const functionKey = payload.contractId && payload.functionName
            ? ACLLib_1.ACLLib.buildPermissionKey(payload.contractId, payload.functionName)
            : undefined;
        switch (payload.action) {
            case IAccessControlList_1.EmergencyActionType.ENABLE_GLOBAL_LOCKDOWN:
                this.emergencyState = {
                    ...this.emergencyState,
                    active: true,
                    activatedBy: actor,
                    activatedAt: ACLLib_1.ACLLib.now(),
                    reason: payload.reason
                };
                break;
            case IAccessControlList_1.EmergencyActionType.DISABLE_GLOBAL_LOCKDOWN:
                this.emergencyState = {
                    ...this.emergencyState,
                    active: false,
                    activatedBy: actor,
                    activatedAt: ACLLib_1.ACLLib.now(),
                    reason: payload.reason
                };
                break;
            case IAccessControlList_1.EmergencyActionType.LOCK_FUNCTION:
                if (!functionKey) {
                    throw new Error('FUNCTION_IDENTIFIER_REQUIRED');
                }
                if (!this.emergencyState.lockedFunctions.includes(functionKey)) {
                    this.emergencyState.lockedFunctions.push(functionKey);
                }
                break;
            case IAccessControlList_1.EmergencyActionType.UNLOCK_FUNCTION:
                if (!functionKey) {
                    throw new Error('FUNCTION_IDENTIFIER_REQUIRED');
                }
                this.emergencyState.lockedFunctions = this.emergencyState.lockedFunctions.filter((lockedFunction) => lockedFunction !== functionKey);
                break;
            case IAccessControlList_1.EmergencyActionType.EMERGENCY_GRANT_ROLE:
                if (!payload.subject || !payload.roleId) {
                    throw new Error('ROLE_AND_SUBJECT_REQUIRED');
                }
                await this.assignRole(payload.subject, payload.roleId, actor, payload.reason);
                break;
            case IAccessControlList_1.EmergencyActionType.EMERGENCY_REVOKE_ROLE:
                if (!payload.subject || !payload.roleId) {
                    throw new Error('ROLE_AND_SUBJECT_REQUIRED');
                }
                await this.revokeRole(payload.subject, payload.roleId, actor, payload.reason);
                break;
            default:
                throw new Error('UNSUPPORTED_EMERGENCY_ACTION');
        }
        this.logAudit(IAccessControlList_1.AuditAction.EMERGENCY_ACTION_EXECUTED, actor, true, payload.reason, { action: payload.action }, { subject: payload.subject, targetId: payload.roleId || functionKey });
        this.eventHandlers.onEmergencyAction?.({ ...this.emergencyState, lockedFunctions: [...this.emergencyState.lockedFunctions] });
        return this.getEmergencyState();
    }
    getRoles() {
        return this.roleManager.getRoles();
    }
    getRole(roleId) {
        return this.roleManager.getRole(roleId);
    }
    getPermissions() {
        return this.permissionManager.getPermissions();
    }
    getPermission(permissionId) {
        return this.permissionManager.getPermission(permissionId);
    }
    getConfig() {
        return {
            ...this.config,
            emergencyCouncil: [...this.config.emergencyCouncil],
            governanceMembers: [...this.config.governanceMembers]
        };
    }
    getEmergencyState() {
        return {
            ...this.emergencyState,
            lockedFunctions: [...this.emergencyState.lockedFunctions]
        };
    }
    getGovernanceMembers() {
        return this.governance.getGovernanceMembers();
    }
    getProposal(proposalId) {
        return this.governance.getProposal(proposalId);
    }
    getProposals() {
        return this.governance.getProposals();
    }
    bootstrapPermissions(actor) {
        for (const [contractId, functions] of Object.entries(PLATFORM_FUNCTIONS)) {
            this.permissionManager.registerFunctionPermissions(contractId, functions, actor);
        }
        for (const contractId of Object.keys(PLATFORM_FUNCTIONS)) {
            const wildcardId = ACLLib_1.ACLLib.normalizeId(`EXECUTE_${contractId}_*`);
            if (!this.permissionManager.getPermission(wildcardId)) {
                this.permissionManager.createPermission({
                    id: wildcardId,
                    contractId,
                    functionName: '*',
                    description: `Execute any function on ${contractId}`,
                    effect: IAccessControlList_1.AccessEffect.ALLOW,
                    riskLevel: 'HIGH'
                }, actor);
            }
        }
        const globalRead = ACLLib_1.ACLLib.normalizeId('EXECUTE_*_*');
        if (!this.permissionManager.getPermission(globalRead)) {
            this.permissionManager.createPermission({
                id: globalRead,
                contractId: '*',
                functionName: '*',
                description: 'Global wildcard access for root roles',
                effect: IAccessControlList_1.AccessEffect.ALLOW,
                riskLevel: 'CRITICAL'
            }, actor);
        }
    }
    bootstrapRoles(actor) {
        PREDEFINED_ROLE_BLUEPRINTS.forEach((role) => {
            const permissionIds = this.getBootstrapPermissionsForRole(role.id);
            this.roleManager.createRole({
                id: role.id,
                label: role.label,
                description: role.description,
                parentRoleIds: role.parents,
                permissionIds,
                metadata: { predefined: 'true' }
            }, actor, true, role.id !== 'SUPER_ADMIN');
        });
    }
    getBootstrapPermissionsForRole(roleId) {
        const wildcard = (contractId) => ACLLib_1.ACLLib.normalizeId(`EXECUTE_${contractId}_*`);
        switch (roleId) {
            case 'SUPER_ADMIN':
                return [ACLLib_1.ACLLib.normalizeId('EXECUTE_*_*')];
            case 'DAO_EXECUTIVE':
            case 'DAO_GOVERNOR':
                return [wildcard('DAO')];
            case 'TREASURY_MANAGER':
            case 'TREASURY_OPERATOR':
                return [wildcard('TREASURY')];
            case 'COMPLIANCE_ADMIN':
            case 'COMPLIANCE_OFFICER':
            case 'REPORTING_MANAGER':
                return [wildcard('REGULATORY')];
            case 'RISK_MANAGER':
                return [wildcard('DAO'), wildcard('TREASURY'), wildcard('LIQUIDITY')];
            case 'SECURITY_ADMIN':
            case 'SECURITY_ANALYST':
            case 'EMERGENCY_RESPONDER':
                return [wildcard('SECURITY')];
            case 'ORACLE_MANAGER':
            case 'ORACLE_OPERATOR':
                return [wildcard('ORACLE')];
            case 'REGISTRY_ADMIN':
                return [wildcard('REGISTRY')];
            case 'LIQUIDITY_MANAGER':
            case 'LIQUIDITY_OPERATOR':
                return [wildcard('LIQUIDITY')];
            case 'STAKING_MANAGER':
            case 'STAKING_OPERATOR':
                return [wildcard('STAKING')];
            case 'FEE_MANAGER':
            case 'FEE_OPERATOR':
                return [wildcard('FEES')];
            case 'AUDITOR':
            case 'SUPPORT_AGENT':
            case 'READ_ONLY_OBSERVER':
                return [ACLLib_1.ACLLib.normalizeId('EXECUTE_REGISTRY_RESOLVE')];
            default:
                return [];
        }
    }
    logAudit(action, actor, success, reason, metadata, overrides = {}) {
        const entry = ACLLib_1.ACLLib.createAuditEntry(action, actor, success, reason, metadata, overrides);
        this.auditTrail.push(entry);
        if (action === IAccessControlList_1.AuditAction.ACCESS_CHECK) {
            this.eventHandlers.onAccessCheck?.(entry);
        }
    }
    recordAccessMeasurement(gasEstimate) {
        this.accessMeasurements.push({
            gasEstimate,
            timestamp: ACLLib_1.ACLLib.now()
        });
    }
    isEmergencyAuthorized(actor) {
        return this.config.emergencyCouncil.includes(actor) || this.roleManager.getEffectiveRoleIds(actor).includes('SECURITY_ADMIN');
    }
    async applyGovernancePayload(proposal) {
        switch (proposal.type) {
            case IAccessControlList_1.GovernanceProposalType.CREATE_ROLE:
                await this.createRole(proposal.payload, proposal.proposer, true);
                break;
            case IAccessControlList_1.GovernanceProposalType.UPDATE_ROLE:
                await this.updateRole(proposal.payload, proposal.proposer);
                break;
            case IAccessControlList_1.GovernanceProposalType.GRANT_ROLE: {
                const payload = proposal.payload;
                await this.assignRole(payload.subject, payload.roleId, proposal.proposer, payload.reason, payload.expiresAt);
                break;
            }
            case IAccessControlList_1.GovernanceProposalType.REVOKE_ROLE: {
                const payload = proposal.payload;
                await this.revokeRole(payload.subject, payload.roleId, proposal.proposer, payload.reason);
                break;
            }
            case IAccessControlList_1.GovernanceProposalType.CREATE_PERMISSION:
                await this.createPermission(proposal.payload, proposal.proposer);
                break;
            case IAccessControlList_1.GovernanceProposalType.ASSIGN_PERMISSION:
                await this.assignPermissionToRole(proposal.payload, proposal.proposer);
                break;
            case IAccessControlList_1.GovernanceProposalType.UPDATE_CONFIG:
                this.applyConfigUpdate(proposal.payload);
                break;
            case IAccessControlList_1.GovernanceProposalType.EMERGENCY_ACTION:
                await this.executeEmergencyAction(proposal.payload, proposal.proposer);
                break;
            default:
                throw new Error('UNSUPPORTED_PROPOSAL_TYPE');
        }
    }
    applyConfigUpdate(payload) {
        const nextGovernanceMembers = payload.governanceMembers ? [...payload.governanceMembers] : [...this.config.governanceMembers];
        this.config = {
            ...this.config,
            ...payload,
            emergencyCouncil: payload.emergencyCouncil ? [...payload.emergencyCouncil] : [...this.config.emergencyCouncil],
            governanceMembers: nextGovernanceMembers
        };
        this.governance.replaceGovernanceMembers(nextGovernanceMembers);
    }
}
exports.AccessControlList = AccessControlList;
//# sourceMappingURL=AccessControlList.js.map